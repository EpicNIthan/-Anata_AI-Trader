...
def main() -> None:
    ...
    paper_ready = directional_accuracy > 0.51 and net_return > 0 and max_drawdown < 0.15
    interesting = directional_accuracy > 0.53 and net_return > 0
    warnings = []
    if directional_accuracy > 0.75:
        warnings.append("Directional accuracy is unrealistically high; check for leakage or overfitting.")
    if net_return <= 0:
        warnings.append("Net return after fees is not positive.")
    if max_drawdown >= 0.15:
        warnings.append("Max drawdown is high for paper-test readiness.")

    metrics = {
        "paper_test_readiness": "PASS" if paper_ready else "FAIL",
        "recommendation": "interesting" if interesting else ("okay_for_paper_test" if directional_accuracy > 0.51 else "not_ready"),
        "directional_accuracy": directional_accuracy,
        "accuracy_interpretation": {
            ">0.51": "okay for paper test",
            ">0.53": "interesting",
            ">0.75": "possible leakage/overfitting warning",
        },
        "net_return_after_fees": net_return,
        "max_drawdown": max_drawdown,
        "max_drawdown_preferred_lt": 0.15,
        "simulated_win_rate": float(sum(1 for ret in traded if ret > 0) / len(traded)) if traded else 0.0,
        "number_of_trades": int(len(traded)),
        "skipped_no_trade_count": int(skipped),
        "average_predicted_return": float(pd.Series(predictions).mean()),
        "average_realized_return_after_fees": float(pd.Series(trade_returns).mean()) if trade_returns else 0.0,
